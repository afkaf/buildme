...
import { useSushiContract, useBambooBarContract } from './useContract'
...
const useBambooBar = () => {
  ...
  const barContract = useBambooBarContract(true) // withSigner
    ...
    async (amount: BalanceProps | undefined) => {
      if (amount?.value) {
        try {
          ...
          return addTransaction(tx, { summary: 'Enter BambooBar' })
        } catch (e) {
          return e
        }
      }
